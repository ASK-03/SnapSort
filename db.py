...
class Database:
    ...
    def _init_schema(self):
        with self.lock:
            c = self.conn.cursor()
            c.execute(
                "CREATE TABLE IF NOT EXISTS images (id INTEGER PRIMARY KEY, path TEXT UNIQUE, modified REAL)"
            )
            c.execute(
                "CREATE TABLE IF NOT EXISTS faces (id INTEGER PRIMARY KEY, last_seen REAL)"
            )
            c.execute(
                "CREATE TABLE IF NOT EXISTS occurrences (image_id INTEGER, face_id INTEGER, x1 INTEGER, y1 INTEGER, x2 INTEGER, y2 INTEGER)"
            )
            self.conn.commit()
    ...
    def insert_occurrence(self, image_path, face_id, box):
        with self.lock:
            ...
            c.execute(
                "INSERT INTO occurrences VALUES(?,?,?,?,?,?)", (img_id, face_id, *box)
            )
            self.conn.commit()
